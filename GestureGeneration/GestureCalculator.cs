    ...
    class GestureCalculator //Analyzer? nah
    {
        ...
        void ConsiderGesture(Touch touch, int touchCount)
        {
            ...
            if (touchCount != 0)
            {
                ...
                _multiGestureTouchCandidates.Add(touch);
        ...
        void EndMultiGesture()
        {
            ...
            if (_multiGestureCandidates.Count == 1)
            {
                ...
                _multiGestureTouchCandidates.Clear();
                return;
            }
            ...
            foreach (KeyValuePair<OneTimeGestureType, List<Touch>> gestureTouches in pendingTouches)
            {
                ...
                if (oddMinority)
                {
                    switch (GestureSettings.MultiGestureInterpretationMode)
                    {
                        case GestureSettings.MultiGestureInterpretationType.IgnoreOddMenOut:
                        case GestureSettings.MultiGestureInterpretationType.IgnoreOddMenOutButSendThemAnyway:
                            continue;
                        case GestureSettings.MultiGestureInterpretationType.RaiseAll:
                ...
                if (GestureSettings.MultiGestureInterpretationMode == GestureSettings.MultiGestureInterpretationType.IgnoreOddMenOutButSendThemAnyway)
                {
                    List<Touch> listToSend = _multiGestureTouchCandidates.ToList();
        ...
        readonly HashSet<Touch> _multiGestureTouchCandidates = new HashSet<Touch>();
        ...
        internal bool IsConsideringForMultiTouch(Touch touch)
        {
            return _multiGestureTouchCandidates.Contains(touch);
        }
    }
}